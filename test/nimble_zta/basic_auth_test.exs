defmodule NimbleZTA.BasicAuthTest do
  use ExUnit.Case, async: true
  import Plug.Test
  import Plug.Conn

  alias NimbleZTA.BasicAuth
  import Plug.BasicAuth, only: [encode_basic_auth: 2]

  defp random_long_id do
    :crypto.strong_rand_bytes(12) |> Base.encode32()
  end
  ...
  test "returns nil when the password is invalid", context do
    authorization = encode_basic_auth(context.username, random_long_id())
    conn = put_req_header(context.conn, "authorization", authorization)
    start_supervised!({BasicAuth, context.options})
